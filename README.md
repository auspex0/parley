...
- **Automatic turns** — New rooms allow four automatic CLI invocations per user message by default. Initial addressed responses and Pair rounds are separate; listening, peer replies, answer returns, catch-up and internal session-recovery attempts consume the shared allowance. Zero allows only initial addressed responses, and Until settled uses a finite safety ceiling. Accepted messages keep their policy and durable usage through Retry/Wake and restart. Existing rooms retain their previous Extra exchanges accounting until explicitly switched in Settings; that policy includes structural deliveries and free answer returns. The composer offers sticky per-room overrides and **Solo** for one selected responder. See [conversation accounting](docs/conversation.md#agent-to-agent-hops--right-of-reply).
