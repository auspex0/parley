# Historical planned-work record — archived 2026-09-18

This is the pre-reconciliation design record, preserved with its original status
claims and the uncommitted September 17 audit. It is **not the active roadmap**;
some claims below are superseded or incorrect. Use [planned-work.md](planned-work.md)
for remaining work and [CHANGELOG.md](../CHANGELOG.md) for released changes.
Old line numbers, branch instructions, free-turn rules and verification labels
describe their historical context, not the current implementation or authorization.
...
### Token-efficiency and latency audit — 2026-09-17

Treat these as one measured-efficiency package, not as claims about how much of any provider's
subscription allowance each item consumed. Use fake providers for correctness and instrumented
real-provider runs only when the user explicitly authorizes quota use.

- **Investigate latency on scoped edits.** The user reported a documentation update taking
  106.8 seconds with 2,977 output tokens (325 reasoning). Its visible activity showed two searches
  and an edit; the first search exited 1, which normally means no matches, not a failed tool.
  Activity labels do not establish patch size, model-step count or where the time was spent.
  The duration measures the whole dispatched run, including prompt preparation and the provider
  invocation; it does not isolate session loading, context processing, tool execution or provider
  latency. Capture available timings and input/cache usage on normal runs before choosing a fix.
  Neither a special small-task mode, automatic effort reduction, extra finish instructions nor
  a hard deadline is established as the remedy for this example. Preserve the user's chosen
  effort and requested scope. Consider a controlled comparison only if ordinary telemetry cannot
  explain the delay and the user authorizes the additional provider use.
- **Attachment delivery ledger.** `stageProviderInputs` can stage a root's files/images again on
  later causal turns, and text previews allow up to 128 KiB per file / 256 KiB per invocation.
  Record attachment IDs delivered to each concrete native session. Re-send after a fresh session,
  on explicit reattachment, or when the current task genuinely requires bytes; otherwise send a
  compact already-available reference. Preserve withdrawal rules and temporary-path validity.
- **Structured missing-session detection.** Resume recovery currently scans combined stdout and
  stderr for provider phrases. A different failure whose assistant/tool output quotes "No
  conversation found with session id" can be misclassified and trigger a paid fresh retry. Prefer
  structured CLI error events or tightly scoped terminal stderr; add an adversarial quoted-phrase
  regression and never retry authentication, quota, transport or tool failures automatically.
- **Honest timeout controls.** Work mode currently raises a configured timeout to at least 15
  minutes, and deep effort raises it to at least 30 minutes; a configured 10-second value therefore
  cannot act as a hard spending/latency guard. Separate a user hard deadline from a recommended
  provider timeout, expose the effective value before launch, and test Stop/deadline outcomes.
- **Reset only on effective execution changes.** Continue the protected-session repair: compare
  the live session's actual permission/sandbox/project/model configuration with the next turn's
  required configuration rather than resetting from coarse room-mode changes. Preserve fail-closed
  behavior on genuine permission changes, hand-edited config and in-flight setting races.
- **Bounded large-backlog delivery.** Ordinary unseen deltas have no total prompt-size ceiling.
  Add byte/token estimates, activity consolidation and a durable catch-up summary for oversized
  histories while retaining every unresolved user request, decision and withholding boundary.
- **Complete usage accounting.** Persist available input, cache-read/write, output and reasoning
  counts per provider attempt, including failed/stopped attempts where the CLI emitted usage.
  Show session reuse/reset reason and automatic-turn ownership beside them. Keep provider-native
  units distinct; do not convert them into a fictional cross-provider cost or quota percentage.

Priority: structured false-retry prevention and attachment delivery first; usage/timing visibility
alongside them; then honest timeout behavior, effective-config reset minimization and oversized
backlog handling. The 106.8-second edit is an investigation case, not proof that any one of these
changes will make document edits faster.
